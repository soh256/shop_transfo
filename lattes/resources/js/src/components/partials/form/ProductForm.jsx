import React, { Fragment, useRef, useState, useEffect } from "react";
import Input from "@material-tailwind/react/Input";
import { useDropzone } from "react-dropzone";
import SelectImage from "../../../assets/images/SelectImage.png";
import ProductService from "../../../Services/Product.service";
import Message from "../../../redux/actions/message/Message";
import { useDispatch } from "react-redux";
import { useHistory } from "react-router";
import { Editor } from "react-draft-wysiwyg";
import "react-draft-wysiwyg/dist/react-draft-wysiwyg.css";
import { EditorState, convertToRaw } from "draft-js";
import draftjsToHtml from "draftjs-to-html";
import ProductAction from "../../../redux/actions/product/Product";

export const ProductForm = React.memo(
    ({ categories, brands, editMode = false }) => {
        const [editing, setEditing] = useState(editMode);
        const [product, setProduct] = useState({ images: [] });
        const [files, setFiles] = useState({});
        const [images, setImages] = useState([]);
        const [loading, setLoading] = useState(false);
        const [editorState, setEditorState] = useState(
            EditorState.createEmpty()
        );

        const dispatch = useDispatch();
        const history = useHistory();

        const handleChange = ({ target }) => {
            const { name, value, files: eFiles } = target;
            if (name === "featured_image") {
                setFiles({ preview: URL.createObjectURL(eFiles[0]) });
                setProduct({ ...product, [name]: eFiles[0] });
            } else if (name === "images") {
                setImages([
                    ...images,
                    {
                        file: eFiles[0],
                        preview: URL.createObjectURL(eFiles[0]),
                    },
                ]);

                setProduct({
                    ...product,
                    images: [...product.images, eFiles[0]],
                });
            } else if (name === "status") {
                setProduct({ ...product, status: !product.status });
            } else if (name === "recommend") {
                setProduct({
                    ...product,
                    recommend: !product.recommend,
                });
            } else if (name === "in_coming") {
                setProduct({
                    ...product,
                    in_coming: !product.in_coming,
                });
            } else {
                setProduct({ ...product, [name]: value });
            }
        };

        const deleteImage = (i) => {
            const newImagesArray = [...images];
            newImagesArray.splice(i, 1);
            setImages(newImagesArray);
        };

        const handleSubmit = async () => {
            setLoading(true);

            const formData = new FormData();
            Object.keys(product).map((field) => {
                if (field === "images") {
                    return product[field].map((image) =>
                        formData.append(`${field}[]`, image)
                    );
                }
                if (
                    (field === "status" &&
                        typeof product[field] === "boolean") ||
                    (field === "status" && typeof product[field] === "number")
                ) {
                    return formData.append(field, product[field] ? 1 : 0);
                }
                if (
                    (field === "in_coming" &&
                        typeof product[field] === "boolean") ||
                    (field === "in_coming" &&
                        typeof product[field] === "number")
                ) {
                    return formData.append(field, product[field] ? 1 : 0);
                }
                if (
                    (field === "recommend" &&
                        typeof product[field] === "boolean") ||
                    (field === "recommend" &&
                        typeof product[field] === "number")
                ) {
                    return formData.append(field, product[field] ? 1 : 0);
                }
                return formData.append(field, product[field]);
            });

            try {
                const { data } = await ProductService.Add(formData);
                dispatch(Message.setMessageSuccess(data.message));
                setImages([]);
                setFiles({});
                setLoading(false);
                history.push("/admin/products");
                dispatch(ProductAction.fetchProduct());
            } catch ({ response }) {
                dispatch(Message.setMessageError(response.data.message));
                setLoading(false);
            }
        };
        const handleEditor = (editorState) => {
            setProduct({
                ...product,
                description: draftjsToHtml(
                    convertToRaw(editorState.getCurrentContent())
                ),
            });
            setEditorState(editorState);
        };

        return (
            <div className="w-full px-3 mt-4 space-y-3 mx-2 flex flex-col mb-3">
                <Input
                    type="text"
                    color="deepPurple"
                    size="sm"
                    outline={true}
                    placeholder="Designation"
                    name="designation"
                    disabled={editing}
                    onChange={handleChange}
                />

                <Input
                    type="text"
                    color="deepPurple"
                    size="sm"
                    outline={true}
                    placeholder="Model"
                    name="model"
                    disabled={editing}
                    onChange={handleChange}
                />

                <Editor
                    editorState={editorState}
                    toolbarClassName="toolbarClassName"
                    wrapperClassName="wrapperClassName"
                    editorClassName="editorClassName"
                    onEditorStateChange={handleEditor}
                />

                <Input
                    type="numeric"
                    color="deepPurple"
                    size="sm"
                    outline={true}
                    placeholder="Quantite"
                    name="qte_stock"
                    disabled={editing}
                    onChange={handleChange}
                />
                <Input
                    type="numeric"
                    color="deepPurple"
                    size="sm"
                    outline={true}
                    placeholder="prix"
                    name="price"
                    disabled={editing}
                    onChange={handleChange}
                />
                <Input
                    type="numeric"
                    color="deepPurple"
                    size="sm"
                    outline={true}
                    placeholder="Remise"
                    name="discount"
                    disabled={editing}
                    onChange={handleChange}
                />
                <Input
                    type="numeric"
                    color="deepPurple"
                    size="sm"
                    outline={true}
                    placeholder="Ratingts"
                    name="ratings"
                    disabled={editing}
                    onChange={handleChange}
                />
                <div className="w-full">
                    <div className="inline-block relative w-full">
                        <select
                            onChange={handleChange}
                            name="category"
                            disabled={editing}
                            className="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline"
                        >
                            <option>~~Selectionnez une Categorie~~</option>
                            {categories?.map((category) => (
                                <option key={category.id} value={category.id}>
                                    {category.nom}
                                </option>
                            ))}
                        </select>
                        <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                            <svg
                                className="fill-current h-4 w-4"
                                xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 20 20"
                            >
                                <path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z" />
                            </svg>
                        </div>
                    </div>
                </div>
                <div className="w-full">
                    <div className="inline-block relative w-full">
                        <select
                            onChange={handleChange}
                            name="brand"
                            disabled={editing}
                            className="block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline"
                        >
                            <option>~~Selectionnez une marque~~</option>
                            {brands?.map((brand) => (
                                <option key={brand.id} value={brand.id}>
                                    {brand.nom}
                                </option>
                            ))}
                        </select>
                        <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-700">
                            <svg
                                className="fill-current h-4 w-4"
                                xmlns="http://www.w3.org/2000/svg"
                                viewBox="0 0 20 20"
                            >
                                <path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z" />
                            </svg>
                        </div>
                    </div>
                </div>
                <div className="w-full p-2 border border-gray-500 rounded-lg border-opacity-60">
                    <label
                        style={{
                            backgroundImage:
                                product.featured_image !== undefined &&
                                JSON.stringify(product) !== "{}" &&
                                `url(${URL.createObjectURL(
                                    product?.featured_image
                                )})`,
                        }}
                        className={`flex flex-col justify-center items-center  h-52 overflow-auto w-full py-2 ${
                            product.featured_image === undefined
                                ? "bg-gray-200 hover:bg-gray-300 "
                                : "bg-contain bg-no-repeat bg-center"
                        } rounded-lg border  cursor-pointer focus:outline-none transition-all duration-150`}
                    >
                        {product.featured_image === undefined && (
                            <>
                                <img
                                    src={SelectImage}
                                    alt="images_paceholder"
                                />
                                <span className="mt-2 text-sm leading-normal text-blue-600">
                                    Choisir une image
                                </span>
                            </>
                        )}
                        <input
                            type="file"
                            className="hidden"
                            accept=".jpg,.jpeg,.png"
                            name="featured_image"
                            onChange={handleChange}
                        />
                    </label>
                </div>
                <div className="w-full h-auto border border-gray-500 rounded-lg p-2 grid grid-cols-4 gap-1">
                    {images.map((image, i) => (
                        <CardImage
                            i={i}
                            key={i}
                            image={image}
                            deleteImage={deleteImage}
                        />
                    ))}
                    <label className="w-32 h-20 py-2 flex flex-col items-center bg-white rounded-md shadow-md tracking-wide uppercase border-2 border-dashed border-gray-500 cursor-pointer hover:bg-gray-100 hover:text-gray-500 text-purple-300 ease-linear transition-all duration-150">
                        <img src={SelectImage} alt="images_paceholder" />
                        <span className="flex justify-center text-xs leading-normal lowercase">
                            Choisir des images
                        </span>
                        <input
                            type="file"
                            className="hidden"
                            accept=".jpg,.jpeg,.png"
                            name="images"
                            onChange={handleChange}
                        />
                    </label>
                </div>
                <div className="mb-3">
                    <div>
                        <div>
                            <label className="inline-flex items-center text-gray-500">
                                <input
                                    type="checkbox"
                                    className="form-checkbox"
                                    checked={product.recommend}
                                    onChange={handleChange}
                                    name="recommend"
                                />
                                <span className="ml-2">Recommendé</span>
                            </label>
                        </div>
                        <div>
                            <label className="inline-flex items-center text-gray-500">
                                <input
                                    type="checkbox"
                                    className="form-checkbox"
                                    checked={product.in_coming}
                                    onChange={handleChange}
                                    name="in_coming"
                                />
                                <span className="ml-2">En Arrivages</span>
                            </label>
                        </div>
                        <div>
                            <span className="text-gray-400 font-medium">
                                Non publier
                            </span>
                            <div className="relative inline-block w-14 mx-2 align-middle select-none">
                                <input
                                    type="checkbox"
                                    name="toggle"
                                    id="Blue"
                                    disabled={editing}
                                    checked={product.status}
                                    name="status"
                                    onChange={handleChange}
                                    className="checked:bg-blue-500 outline-none focus:outline-none right-4 checked:right-0 duration-200 ease-in absolute block w-10 h-6 rounded-full bg-white  appearance-none cursor-pointer"
                                />
                                <label
                                    htmlFor="Blue"
                                    className="block overflow-hidden h-6  rounded-full bg-gray-300 cursor-pointer"
                                ></label>
                            </div>
                            <span className="text-gray-400 font-medium">
                                Publier
                            </span>
                        </div>
                    </div>
                </div>
                <button
                    onClick={handleSubmit}
                    className="flex w-full mt-5  space-x-3 text-sm font-medium  bg-blue-500 rounded-md  text-white  justify-center items-center p-1   hover:bg-blue-600  transition duration-300 ease-in-out focus-within:outline-none py-2"
                >
                    {loading && "Loading..."}
                    {!loading && "Enregistrez"}
                </button>
            </div>
        );
    }
);

const CardImage = React.memo(({ image, deleteImage, i }) => {
    const [show, setShow] = useState(false);
    return (
        <div
            className="w-32 h-20 rounded-md shadow-md overflow-hidden bg-cover bg-center relative"
            style={{ backgroundImage: `url(${image.preview})` }}
            onMouseEnter={() => setShow(true)}
            onMouseLeave={() => setShow(false)}
        >
            {show && (
                <div className=" flex space-x-4  bg-black bg-opacity-80 p-2 transition duration-200 left-0 top-0 bottom-0 right-0 absolute">
                    <span
                        onClick={() => deleteImage(i)}
                        className="text-white hover:text-gray-900 transition duration-200"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            className="h-6 w-6"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                        >
                            <path
                                strokeLinecap="round"
                                strokeLinejoin="round"
                                strokeWidth="2"
                                d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"
                            />
                        </svg>
                    </span>
                </div>
            )}
        </div>
    );
});
